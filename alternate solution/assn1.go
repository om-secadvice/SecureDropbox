...
//returnEncryptedUser - Returns marshaled GenericFile struct with encrypted content and its hmac which can be directly stored
func (userdata *User) returnEncryptedUser() (file []byte, err error) {
	//Prepare encryption params
	...
	hash := userlib.NewSHA256()
	hash.Write(b(password))
	copy(aesKey, hash.Sum(nil))
	genFile := GenericFile{}

	//Return Encrypted JSON file
	return genFile.encryptCFB(aesKey, b(username+password), userdata)
...
func InitUser(username string, password string) (userdataptr *User, err error) {
	...
	file, err := userdata.returnEncryptedUser()
	if err != nil {
		return nil, err
	}
	//Store File
...
func (userdata *User) StoreFile(filename string, data []byte) {
	...
	contentFile, _ := new(GenericFile).encryptCFB(aesKey, macKey, data)

	//FileMeta Preparation
	//Calculate File Location Using new UUID every time
	hash = userlib.NewSHA256()
	hash.Write(b(uuid.New().String()))
	hash.Write(macKey)
	fileloc := enc(hash.Sum(nil))
...
func (userdata *User) AppendFile(filename string, data []byte) (err error) {
	...
	contentFile, e := new(GenericFile).encryptCFB(contentKey, macKey, data)
	if e != nil {
		return e
	}
...
func (userdata *User) LoadFile(filename string) (data []byte, err error) {
	...
	for _, loc := range metafile.ListOfUUIDs {
		...
		genFile = GenericFile{}
		msgPart, er := genFile.decryptCFB(dec(header.ContentKey), dec(header.MACKey), buff)
		if err != nil {
			err = er
			return
		}
